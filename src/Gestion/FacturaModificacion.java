...
public class FacturaModificacion extends Frame implements WindowListener, ActionListener
{
	...
	public FacturaModificacion()
	{
		...
		btncompl.addActionListener(new ActionListener() {
			...
			private void CompletarPrecio(Connection con2, int parseInt, TextField txtFecha) throws ParseException {
				...
				try {
					...
					while (rs.next()) 
					{
						...
						java.util.Date date = parseador.parse(rs.getString("fechaFactura"));
	...
	private void rellenarchofac(Connection con, Choice chofac2) throws ParseException {
		...
		try {
			...
			while (rs.next()) 
			{
				...
				chofac2.add(rs.getInt("idFacturas")+
						"-"+fecha+
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if(objetoPulsado.equals(btnSi))
		{
			...
			try {
				date = parseador.parse(txtFecha.getText());
			} catch (ParseException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			...
			int respuesta = modificar(con,Integer.parseInt(fact[0]), fecha,txtimporte.getText());
			// Mostramos resultado
			if(respuesta == 0)
			{
